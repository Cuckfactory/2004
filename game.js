...
(() => {
  ...
  const PICTURES = [
    { id: "dsc00031", name: "DSC00031.JPG", src: "assets/pictures/DSC00031.jpg", icon: "🌃", note: "2:07 AM · flash on · nobody remembers taking it" },
    { id: "bedroom", name: "bedroom_flash.jpg", src: "assets/pictures/bedroom_flash.jpg", icon: "🛏️", note: "cheap lights, CRT glow, one chair facing the wrong way" },
    { id: "lan", name: "lan_party_final.jpg", src: "assets/pictures/lan_party_final.jpg", icon: "🖥️", note: "four beige towers · one extension cord · absolutely no sleep" },
    { id: "school", name: "schooltrip.jpg", src: "assets/pictures/schooltrip.jpg", icon: "🚌", note: "date stamp says 2004. nobody in the photo is looking at the camera" },
    { id: "jess", name: "do_not_send_to_jess.jpg", src: "assets/pictures/do_not_send_to_jess.jpg", icon: "🌸", note: "the filename is doing most of the work here" },
    { id: "horse", name: "horse.jpg", src: "assets/pictures/horse.jpg", icon: "🐴", note: "there is no horse on your computer" },
  ];
  ...
  const CONTACTS = [
    ...
    },
    {
      ...
      story: 0,
      lines: {
        hello: ["you added me first.", "wrong window.", "oh. you still have this account."],
        chat: [
          "i was going to ask you the same thing",
          "your computer keeps better secrets than you do",
          "don't close My Pictures yet",
          "the modem remembers more than the browser",
          "there are easier ways to make a bad decision. you installed messenger for one",
        ],
        music: ["leave it on. the quiet part is doing something", "winamp knows. i don't ask it questions"],
        bye: ["you can sign off. the file will still be there", "goodnight. don't delete anything you didn't create"],
      },
      replies: ["who are you", "what do you want", "check my computer", "should I buy this", "you're weird"],
    },
    {
      id: "kate",
      name: "katie.p",
  ...
  function load() {
    try {
      ...
      Object.assign(state, {
        ...
        pictures: Array.isArray(d.pictures) ? d.pictures : state.pictures,
      });
  ...
  function unlockPicture(id, from = "wrong_number") {
    const p = pictureById(id);
    if (!p || state.pictures.includes(id)) return false;
    state.pictures.push(id);
    save();
    renderPictures();
    ping();
    toast("My Pictures", "New picture received: " + p.name);
    const picWin = document.getElementById("pictures");
    if (picWin && !picWin.classList.contains("hidden")) {
      const st = $("#pictures-status");
      if (st) st.textContent = `${state.pictures.length} object(s) · received from ${from}`;
    }
    return true;
  }
  ...
  function renderPictures() {
    ...
    if (!grid) return;
    const unlocked = state.pictures.map(pictureById).filter(Boolean);
    grid.innerHTML = unlocked.map((p) => `
      <button class="picture-item" data-picture="${p.id}" title="${escapeHtml(p.name)}">
        <span class="picture-thumb">
          <img src="${p.src}" alt="${escapeHtml(p.name)}" />
          <span class="picture-fallback">${p.icon}</span>
        </span>
        <span class="picture-name">${escapeHtml(p.name)}</span>
      </button>`).join("");

    grid.querySelectorAll(".picture-item").forEach((item) => {
      const img = item.querySelector("img");
      if (img) img.addEventListener("error", () => item.classList.add("missing-image"), { once: true });
      item.addEventListener("click", () => previewPicture(item.dataset.picture));
      item.addEventListener("dblclick", () => previewPicture(item.dataset.picture, true));
    });
    const st = $("#pictures-status");
    if (st) st.textContent = `${unlocked.length} object(s)`;
  }

  function previewPicture(id, large = false) {
    const p = pictureById(id);
    const box = $("#pictures-preview");
    if (!p || !box) return;
    box.innerHTML = `
      <div class="preview-card ${large ? "large" : ""}">
        <div class="preview-media">
          <img src="${p.src}" alt="${escapeHtml(p.name)}" />
          <div class="preview-fallback">${p.icon}</div>
        </div>
        <div class="preview-copy"><b>${escapeHtml(p.name)}</b><span>${escapeHtml(p.note)}</span></div>
      </div>`;
    const img = box.querySelector("img");
    if (img) img.addEventListener("error", () => box.classList.add("missing-preview"), { once: true });
    box.classList.remove("missing-preview");
  }

  function weirdReply(c, lower) {
    const tr = TRACKS[state.trackIndex];
    const live = CONTACTS.filter((x) => x.token && x.online && x.phase !== "whisper" && x.phase !== "glass");
    const focused = quote.focus ? pairName(quote.focus) : "";
    const target = focused || (live[0] && live[0].name) || "";

    if (/horse/.test(lower)) {
      unlockPicture("horse");
      return "there isn't a horse on your computer.";
    }
    if (/jess/.test(lower)) {
      const fresh = unlockPicture("jess");
      return fresh ? "check My Pictures. don't send that one to jess." : "the filename already told you what not to do.";
    }
    if (/lan|server|counter.?strike|game/.test(lower)) {
      const fresh = unlockPicture("lan");
      return fresh ? "i found the LAN photo. check My Pictures. the beige towers were louder than the people." : "the extension cord survived. somehow.";
    }
    if (/school|trip|2004|remember|old/.test(lower)) {
      const fresh = unlockPicture("school");
      return fresh ? "something from 2004 just appeared in My Pictures. you are not looking at the camera." : "memory is just a badly named file with no backup.";
    }
    if (/picture|photo|image|file|computer|folder/.test(lower)) {
      const fresh = unlockPicture("bedroom");
      return fresh ? "check My Pictures." : pick(["you already checked. that doesn't mean you saw it.", "the folder changed before you opened it.", "one of those files wasn't there at 2:03."]);
    }
    if (/who are you|who r u|who're you|your name|what are you/.test(lower)) {
      return pick(["you added me first.", "wrong number.", "i'm the account you don't remember deleting.", "if I tell you, you'll rename the file."]);
    }
    if (/buy|sell|trade|coin|token|pump|entry|ape|\$[a-z0-9]+/.test(lower)) {
      return target
        ? `${target} is on the glass. you have ${state.memory} memory and ${state.attention.toFixed(1)} attention. finish one bad decision before starting another.`
        : `there is nothing green enough to blame right now. you still have ${state.memory} memory. keep it.`;
    }
    if (/time|clock|late|morning|dawn/.test(lower)) {
      return `${clockLabel()}. the clock is the least strange thing on this desktop.`;
    }
    if (/song|music|winamp|playing|listen/.test(lower)) {
      return `you're playing ${tr.name}. leave it on. the quiet part is doing something.`;
    }
    if (/memory|attention|score/.test(lower)) {
      return `memory: ${state.memory}. attention: ${state.attention.toFixed(1)}. none of that explains why I'm still online.`;
    }
    if (/hello|hi|hey|yo/.test(lower)) {
      return pick(["wrong window.", "hi. don't make this normal.", "oh. you're still here.", "you type exactly like I remember."]);
    }
    if (/bye|goodnight|gn|sleep|sign off/.test(lower)) {
      return pick(c.lines.bye);
    }

    if (c.story === 3 && !state.pictures.includes("bedroom")) {
      unlockPicture("bedroom");
      return "check My Pictures. don't ask why yet.";
    }
    if (c.story >= 7 && !state.pictures.includes("horse")) {
      unlockPicture("horse");
      return "I fixed the folder.";
    }

    return pick([
      "that's not the question you typed the first time.",
      "your cursor hesitated before you sent that.",
      "I can hear the hard drive thinking.",
      "there are six people online and somehow you picked me.",
      "close the window if you want. I don't live in the window.",
      "the weird part is that this answer will feel less weird in five minutes.",
      "you keep asking the screen like the screen came here on purpose.",
      "the modem made that sound again. no, not that sound.",
    ]);
  }
  ...
  function npcReply(c, lower) {
    let line;
    if (c.id === "wrong") {
      line = weirdReply(c, lower);
    } else if (/a\/s\/l/.test(lower)) {
  ...
  const tape = { last: 0, seen: new Set(), nominees: new Set(), log: [], social: new Map() };
  const SOCIAL_FEED_URL = "";
  const MARKET_SIGN_IN = 55;
  const WHISPER_MS = 10 * 60 * 1000;
  ...
  function tokenPhaseStatus(sym, phase) {
    ...
    if (phase === "whisper") return s + " · people are talking";
    if (phase === "glass") return s + " · on the glass";
    if (phase === "early") return s + " · volume before price";
    if (phase === "loud") return s + " · getting loud";
    if (phase === "away") return s + " · away";
    if (phase === "faded") return s + " · chatter faded";
    if (phase === "off") return s + " · signed off";
    return s + " · signed in";
  }
  function entityConfidence(ev) {
    if (!ev) return 0;
    if (ev.ca) return 1;
    if (ev.official) return 0.9;
    if (ev.name && ev.cashtag) return 0.7;
    if (ev.cashtag) return 0.25;
    return 0;
  }
  function socialFloors(s) {
    return !!(s && s.posts5 >= 12 && s.creators5 >= 8 && s.posts1h >= 20);
  }
  function socialBoostOf(s) {
    if (!s || !socialFloors(s) || entityConfidence(s) < 0.65) return 0;
    const creator = Math.min(3, ratio(s.creators5 * 12, s.creators1h || s.creators5));
    const mention = Math.min(3, ratio(s.posts5 * 12, s.posts1h));
    const breadth = s.posts5 ? s.creators5 / s.posts5 : 0;
    const raw = 0.45 * creator + 0.25 * mention + 0.2 * Math.min(1, breadth / 0.35) + 0.1 * (s.quality || 0.4) - (s.spam || 0);
    return Math.max(0, Math.min(15, raw * 5));
  }
  function loadTapeLog() {
    try { tape.log = JSON.parse(localStorage.getItem("signed-in-tape-log") || "[]"); } catch (e) { tape.log = []; }
    if (!Array.isArray(tape.log)) tape.log = [];
  }
  function saveTapeLog() {
    try { localStorage.setItem("signed-in-tape-log", JSON.stringify(tape.log.slice(-200))); } catch (e) {}
  }
  function logTapeEvent(c, event) {
    ...
    tape.log.push({
      ...
      key: c.hit && c.hit.key,
      phase: c.phase,
      px: Number(p && p.priceUsd) || 0,
      market: c.hit && c.hit.marketScore,
      social: c.hit && c.hit.socialBoost,
      fwd: {},
    });
    saveTapeLog();
  }
  function updateTapeFwd(pool) {
    const now = Date.now();
    tape.log.forEach((row) => {
      if (!row.px || !row.key) return;
      let p = null;
      pool.forEach((v) => {
        const k = v.p && (v.p.pairAddress || v.p.baseToken && v.p.baseToken.address);
        if (k === row.key) p = v.p;
      });
      if (!p || !p.priceUsd) return;
      const ret = ((Number(p.priceUsd) - row.px) / row.px) * 100;
      const age = now - row.t;
      if (age >= 5 * 60 * 1000 && row.fwd.m5 == null) row.fwd.m5 = +ret.toFixed(2);
      if (age >= 15 * 60 * 1000 && row.fwd.m15 == null) row.fwd.m15 = +ret.toFixed(2);
      if (age >= 30 * 60 * 1000 && row.fwd.m30 == null) row.fwd.m30 = +ret.toFixed(2);
      if (age >= 60 * 60 * 1000 && row.fwd.h1 == null) row.fwd.h1 = +ret.toFixed(2);
      row.fwd.max = +Math.max(row.fwd.max == null ? ret : row.fwd.max, ret).toFixed(2);
      row.fwd.min = +Math.min(row.fwd.min == null ? ret : row.fwd.min, ret).toFixed(2);
    });
    saveTapeLog();
  }
  function scorePair(p, nominated, socialEv) {
    ...
    if (liq < 15000) return null;
    if (vol1 < 4000 && vol5 < 1200) return null;
    if (h24 > 400) return null;
    if (px1 >= 140) return null;

    const quietPx = Math.abs(px1) <= 8 && Math.abs(px5) <= 3.5;
    const early =
      activity >= 2.1 && tx5 >= 12 && buyAccel >= 1.35 && buys5 > sells5 && quietPx;
    const loud =
      activity >= 1.8 && tx5 >= 14 && buySkew >= 0.55 && Math.abs(px5) >= 2.5 && Math.abs(px1) < 55;
    const grind = px1 >= 10 && px1 < 65 && px5 >= 2.5 && activity >= 1.25 && liq >= 25000;

    let kind = "watch";
    let phase = "watch";
    let marketScore = Math.max(0, Math.min(54, 18 + activity * 8 + buySkew * 10));
    if (early) {
      kind = "early";
      phase = "early";
      marketScore = Math.min(96, 70 + activity * 8 + buySkew * 12);
    } else if (loud) {
      kind = "loud";
      phase = "loud";
      marketScore = Math.min(88, 58 + activity * 6 + Math.min(Math.abs(px5), 12));
    } else if (grind) {
      kind = "grind";
      phase = "in";
      marketScore = Math.min(80, 48 + Math.min(px1, 25) + activity * 4);
    }

    const sBoost = socialBoostOf(socialEv);
    let score = marketScore;
    if (marketScore >= MARKET_SIGN_IN) score = marketScore + Math.min(15, sBoost);

    let note;
    if (kind === "early") {
      note = `volume before price. 5m tx ${tx5} (${buys5}b/${sells5}s). tx ${txAccel.toFixed(1)}×. price only ${px1 >= 0 ? "+" : ""}${px1.toFixed(1)}%.`;
    } else if (kind === "loud") {
      note = `getting loud. buy accel ${buyAccel.toFixed(1)}× · vol ${volAccel.toFixed(1)}× · 5m ${px5 >= 0 ? "+" : ""}${px5.toFixed(1)}%.`;
    } else if (kind === "grind") {
      note = `slope. +${px1.toFixed(1)}% / 1h. not a firework.`;
    } else if (sBoost > 0) {
      note = "people are talking. the tape has not confirmed. social can knock. only the market signs in.";
    } else if (nominated) {
      note = "nominated for scanning only. paid attention can never create a visible print.";
    } else {
      note = "not enough market to sign in.";
    }

    const canSign = score >= MARKET_SIGN_IN && (kind === "early" || kind === "loud" || kind === "grind");
    const canWhisper = !canSign && sBoost > 0;
    if (!canSign && !canWhisper) return null;

    if (canWhisper && !canSign) {
      kind = sBoost > 0 ? "whisper" : "glass";
      phase = kind;
    }
    return {
      kind, phase, score, marketScore, socialBoost: sBoost, p, note, nominated: !!nominated,
  ...
  function signTokenOff(c, how) {
    ...
    if (how === "away") {
      ...
      c.phase = "faded";
      c.pic = "⚫";
      c.status0 = tokenPhaseStatus(c.sym || c.name.replace("$", ""), "faded");
  ...
  function upsertTokenContact(hit) {
    ...
    if (!c) {
      c = {
        ...
        pic: hit.phase === "whisper" ? "🟣" : hit.phase === "glass" ? "🟣" : "🟢",
        genre: "idm",
        persona: "token",
        token: true,
    ...
    c.online = true;
    c.mood = hit.phase === "whisper" || hit.phase === "glass" ? "away" : "online";
    c.pic = hit.phase === "whisper" || hit.phase === "glass" ? "🟣" : "🟢";
  ...
  async function scanTape() {
    ...
    try {
      ...
      if (SOCIAL_FEED_URL) {
        const feed = await fetch(SOCIAL_FEED_URL).then((r) => r.json()).catch(() => []);
        (Array.isArray(feed) ? feed : []).forEach((ev) => {
          const mint = ev.mint || ev.tokenAddress || ev.ca;
          if (!mint) return;
          ev.ca = ev.ca || mint;
          tape.social.set(String(mint).toLowerCase(), ev);
          if (!tape.nominees.has(mint) && entityConfidence(ev) >= 0.65 && socialFloors(ev)) {
            tape.nominees.add(mint);
            needFetch.push(mint);
          }
        });
        const more = await Promise.all(needFetch.slice(-6).map(pairFromToken));
        more.forEach((p) => {
          if (!p) return;
          const key = p.pairAddress || p.baseToken?.address;
          if (key && !pool.has(key)) pool.set(key, { p, nominated: true });
      ...
      pool.forEach(({ p, nominated }) => {
        const mint = (p.baseToken && p.baseToken.address || "").toLowerCase();
        const hit = scorePair(p, nominated, tape.social.get(mint));
      ...
      CONTACTS.filter((c) => c.token && (c.online || c.mood === "away")).forEach((c) => {
        ...
        c.hit = hit;
        c.pair = hit.p;
        if ((prev === "whisper" || prev === "glass") && (hit.phase === "early" || hit.phase === "loud" || hit.phase === "in")) {
          c.phase = hit.phase;
          c.pic = "🟢";
          c.mood = "online";
          c.status0 = tokenPhaseStatus(c.sym, hit.phase);
          npcSay(c, c.name + " has signed in.");
          logTapeEvent(c, "signed-in-from-whisper");
        } else if (hit.phase === "loud" && prev !== "loud" && c.online && prev !== "whisper") {
          c.phase = "loud";
          c.status0 = tokenPhaseStatus(c.sym, "loud");
        } else if (hit.phase === "early" && c.online && prev !== "whisper") {
          c.phase = "early";
          c.status0 = tokenPhaseStatus(c.sym, "early");
        }
      });
      CONTACTS.filter((c) => c.token && (c.phase === "whisper" || c.phase === "glass") && c.signedAt && Date.now() - c.signedAt > WHISPER_MS).forEach((c) => {
        signTokenOff(c, "faded");
        logTapeEvent(c, "chatter-faded");
      });
      hits.sort((a, b) => b.score - a.score);
      const signHits = hits.filter((h) => h.phase === "early" || h.phase === "loud" || h.phase === "in");
      const whisperHits = hits.filter((h) => h.phase === "whisper" || h.phase === "glass");
      const pickOne = signHits.find((h) => {
        if (!tape.seen.has(h.key)) return true;
        const ex = CONTACTS.find((x) => x.token && x.hit && x.hit.key === h.key);
        return ex && (!ex.online || ex.phase === "away" || ex.phase === "off" || ex.phase === "faded" || ex.phase === "whisper" || ex.phase === "glass");
      });
      if (pickOne) {
        ...
        if (fresh) {
          npcSay(c, c.name + " has signed in.");
          logTapeEvent(c, "signed-in");
        } else if (reactivated && c.phase !== "whisper" && c.phase !== "glass") {
          npcSay(c, pick([c.name + " has signed in again.", c.name + " is back."]));
          logTapeEvent(c, "signed-in-again");
        }
      } else {
        const w = whisperHits.find((h) => !tape.seen.has(h.key));
        if (w) {
          tape.seen.add(w.key);
          const { c, fresh } = upsertTokenContact(w);
          if (fresh) {
            npcSay(c, w.phase === "glass" ? c.name + " is on the glass." : c.name + " — people are talking.");
            logTapeEvent(c, "whisper");
          }
        }
      }
      renderMessenger();
  ...
  function patienceTick(dtMin) {
    CONTACTS.forEach((c) => {
      ...
      if (c.id === "radio" || c.id === "tape" || c.id === "wrong") return;
      if (c.token && (c.phase === "whisper" || c.phase === "glass")) return;
  ...
  async function wizardContinue() {
    ...
    try {
      ...
      if (built.mode === "v2" && built.requestId && JUPITER_API_KEY) {
        ...
        if (!execRes.ok || !exec || String(exec.status || "").toLowerCase() !== "success") {
          throw new Error((exec && (exec.error || exec.errorMessage || exec.code)) || "execute failed");
        }
      } else {
  ...
  function boot() {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        try {
          if (!localStorage.getItem("signed-in-wrong-ping")) {
            const wrong = CONTACTS.find((c) => c.id === "wrong");
            if (wrong) npcSay(wrong, "you added me first.");
            localStorage.setItem("signed-in-wrong-ping", "1");
          }
        } catch (e) {}
      }, 6200);
  ...
  window.SignedIn = { openApp, minimize, closeWin, playTrack, renderPictures, unlockPicture, state };
